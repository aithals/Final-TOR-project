...
void routerroutine(int a[], int nrot, struct sockaddr_in gserv_addr,char ipadd[])
{
    ...
    char buf[100];
     ...
     sprintf(buf,"/sbin/ifconfig eth%d |grep 'inet '|cut -d ' ' -f 10-11|awk '{ print $1 }'",nrot);
     fp=popen(buf,"r");
     if (fp == NULL) {
     ...
     printf(">>>>>>>>>>>>>%s>>>>>>>>>>>\n",ipad);*/
		...
		/*raw socket for tcp*/
			...
			printf("TCP port izzz%d \n", ntohs(rawtcp_addr.sin_port));
		/*raw socket for tcp ends*/

		/*provisioning select function*/
			fd_set readfds;
			int maxfd;
			//int maxfd = rawfd > newsockfd1 ? rawfd:newsockfd1;
			//maxfd++;
			if(rawfd > newsockfd1)
			{
				if(rawfd>rawtcpfd)
			...
			while(1)
			{
					...
					if(a[0]==9)
					{
						if(pktsent==1)
						{
							...
							*type=0x92;
							...
							if (sendto(newsockfd1, sendbuf, 3, 0, (struct sockaddr *)&pvaddr, clilen) < 0) {
								perror("sendto failed");
								return;
							}*/
							exit(0);
				...
				if(a[0]==5)
				{
					...
					if (rv > 0)
					{
						...
						//printf("Router nalli: ip protocol %d\n",ip->protocol);
						...
						if(*type == 0x52)
						{
							...
							incmid=msg->circid;
							printf("Router nalli:type is %x port num %d, hex : %02x \n",*type,msg->portnum,msg->circid);
							if(newid==0)
							{
								...
								*typen=0x53;
								...
								printf("msg contents:  0x%02x%04x  \n",*type,msgn->circid);
						...
						else if(*type == 0x51)
						{
							...
							/*checksum calculation*/
								...
								icmppkt[8]='\0';*/
									printf("size recvd at last router is %d",rv);
								...
								else
								{
									...
									ip->saddr=soaddr.s_addr;/*what about checksum??*/
									msgr->circid=myid;
									if (sendto(newsockfd1, recbuf, rv, 0, (struct sockaddr *)&sender_addr, clilen) < 0) {
										perror("sendto failed");
										exit(0);
				...
				if(FD_ISSET(rawfd,&readfds))
				{
					...
					printf("recvd icmp packet: size is %d type is %d\n",rvm,icmp1->type);
					printf("ICMP from raw sock, src: %s, dst: %s, type: %d \n",tempo1,tempo2,icmp1->type);
					/*common code for raw socket interface starts here*/
...
/*inmcid prob here*/	fprintf(rouf2,"incoming packet, src: %s, dst: %s, outgoing circuit: 0x%02x\n",tempo1,tempo2,incmid);
						...
						*type=0x64;
						...
						printf("checking decryption : before encryption protocol is %d\n",ipk->protocol);
						pts=(unsigned char*) (sendbuf + sizeof(struct iphdr)+1+sizeof(struct mycntrlmsgr));
						...
						printf("checking decryption : protocol is %d\n",ip->protocol);
					...
					else if(a[0]==8)
					{
						...
						printf("my key is %s , whichtupp value is %d \n",tempo,whichtupple);
						char sendbuf[1000];
						unsigned char *pts;
						char *ptr=icmppkt;
						memset(sendbuf,0,sizeof(sendbuf));
...
/*incmid prob here*/fprintf(rouf2,"incoming TCP packet, src IP/port: %s:%d, dst IP/port: %s:%d, seqno: %lu, ackno: %lu, outgoing circuit: 0x%02x\n",tempo1,ntohs(tcp->source),tempo2,ntohs(tcp->dest),(long unsigned)ntohl(tcp->seq),(long unsigned)ntohl(tcp->ack),ntohs(inocmid));
					...
					//fprintf(prof1,"incoming TCP packet, circuit incoming: 0x01, src IP/port: %s:%d, dst IP/port: %s:%d, seqno: %lu, ackno: %lu\n",str1,ntohs(tcp->source),str,ntohs(tcp->dest),ntohl(tcp->seq),ntohl(tcp->ack));
					//fflush(prof1);
					...
					*type=0x64;
					...
					else if(a[0]==8)
					{
						...
						inet_ntop(AF_INET, &(soaddr1), tempo1, INET_ADDRSTRLEN);
						inet_ntop(AF_INET, &(dsaddr1), tempo2, INET_ADDRSTRLEN);
						printf("source ip is %s and dest port is %d\n",tempo1,tcp->dest);
										...
										else
										{
											...
											if(tcp->source == current->intupple.src_port)
											{
												if(tcp->dest == current->intupple.dest_port)
												{
													//different =0;
													whichtupple=current->intupple.keyref;
													printf("got the tupple and brokefrom loop after %d iteration \n",whichtupple);
													break;
												}
											}
										}
					...
					/*>>>*/	msgr->circid=current->intupple.incm;
						...
						ip = (struct iphdr*)clear_crypt_text;
						printf("checking decryption : protocol is %d\n",ip->protocol);
						printf("got packet in the last router: prev addr port is %d , %x\n",ntohs(current->intupple.pv_addr.sin_port),current->intupple.incm );
						if (sendto(newsockfd1, sendbuf, 23+crypt_text_len, 0, (struct sockaddr *)&(current->intupple.pv_addr), sizeof(back_addr)) < 0) {
								perror("sendto failed");
								exit(0);
						}
