...
static short circuit=0x01;/*for keeping track of number of circuits being created(one for each new tupple)*/

int creatckt_s8(int sockfd,int nrot,int nhops,struct sockaddr_in cli_addr[],FILE *prof1,unsigned char *rrkeys[17])
{
...
randomagain:hopselk[i]=rand() % nrot;
	...
	/*key sending code*/
		...
		*typek=0x65;
		...
		fprintf(prof1,"new-fake-diffe-hellman, router index: %d, circuit outgoing: 0x%02x, key: 0x",hopselk[i]+1,circuit);
		fflush(prof1);
		j=0;
		while(j<16)
		{
